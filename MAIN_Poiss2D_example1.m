...
rhs=@(x,y) 6.*x.*y.*(1-y)-2.*x.^3;              % right-hand-side function
valw=@(y) [ones(size(y)), zeros(size(y)), y.*0];            % values at west boundary {\alpha,\beta,\gamma}
vale=@(y) [ones(size(y)), zeros(size(y)), y.*(1-y)];        % values at east boundary {\alpha,\beta,\gamma}    
...
ua=Yp.*(1-Yp).*Xp.^3;    % analytical solution
surf(Xp,Yp,ua)
shading interp
hold on
