...
% Neumann-Periodic boundary conditions are imposed
...
Lx=1;                                           % domain length
Ly=1;                                           % domain length
nx=40;                                          % number of nodes in x dir.
ny=30;                                          % number of nodes in y dir.
pbc=[true(1),false(1)];                         % periodic boundary condition flag [w-e,s-n]
refx='lin';                                     % grid refinement law x dir.
refy='tanh-e';                                  % grid refinement law y dir.
rhs=@(x,y) -17/2*pi^2.*sin(pi/2.*y).*sin(pi.*x).*cos(pi.*x);              % right-hand-side function
