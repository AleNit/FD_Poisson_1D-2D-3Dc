...
% Dirichelet boundary conditions are imposed in the r and z direction.
...
rhs=@(r,t,z) 6.*r.*z.*cos(t);                                               % right-hand-side function
valz0=@(t,r) cat(3,ones(nt-1,nr-1), zeros(nt-1,nr-1), r.*0);                % values at z=0 boundary
valz1=@(t,r) cat(3,ones(nt-1,nr-1), zeros(nt-1,nr-1), r.^3.*cos(t).^3);     % values at z=Lz boundary
valr0=@(t,z) cat(3,ones(nt-1,nz-1), zeros(nt-1,nz-1), z.*0);                % values at r=0 boundary
valr1=@(t,z) cat(3,ones(nt-1,nz-1), zeros(nt-1,nz-1), z.*cos(t).^3);        % values at r=Lr boundary
...
ua=@(r,t,z) r.^3.*z.*cos(t).^3;     % analytical solution
