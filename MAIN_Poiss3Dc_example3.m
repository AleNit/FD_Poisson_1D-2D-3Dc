...
% Boundary conditions: r --> (Neumann-Neumann),  z --> (Dirichelet)
...
rhs=@(r,t,z) -( r.*(9*pi^2.*z.^2+8).*sin(pi/2.*r) - 18*pi.*z.^2.*cos(pi/2.*r) )./(36.*r.*z.^(4/3)); 
valz0=@(t,r) cat(3,ones(nt-1,nr-1), zeros(nt-1,nr-1), r.*0);                % values at z=0 boundary
valz1=@(t,r) cat(3,ones(nt-1,nr-1), zeros(nt-1,nr-1), sin(pi/2.*r));                % values at z=Lz boundary
...
ua=@(r,t,z) z.^(2/3).*sin(pi/2.*r);     % analytical solution
